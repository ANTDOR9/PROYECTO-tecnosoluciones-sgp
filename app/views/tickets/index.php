...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="fw-bold mb-1">Tickets</h5>
        <small style="color:var(--text-secondary);">Gestiona las solicitudes y soporte a clientes</small>
    </div>
    <a href="/PROYECTO-tecnosoluciones-sgp/public/tickets/create" class="btn btn-primary">
        ...
        <div class="row g-3 align-items-center">
            <div class="col-md-4">
                <div class="input-group">
                    ...
                    <input type="text" id="searchInput" class="form-control" placeholder="Buscar ticket...">
                </div>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-2 text-end">
                <small style="color:var(--text-secondary);">
                    Total: <strong style="color:var(--text-primary);"><?= count($tickets) ?></strong>
                </small>
            </div>
        </div>
    </div>
                ...
                <?php else: ?>
                    <?php foreach ($tickets as $t): ?>
                        <tr data-estado="<?= $t['estado'] ?>" data-prioridad="<?= $t['prioridad'] ?>">
                            ...
                            <td><?= $t['proyecto_nombre'] ? htmlspecialchars($t['proyecto_nombre']) : '<span style="color:var(--text-secondary);">-</span>' ?></td>
                            <td>
                            ...
                            <td>
                                <div class="d-flex gap-2">
                                    <a href="/PROYECTO-tecnosoluciones-sgp/public/tickets/show/<?= $t['id'] ?>"
                                       class="btn btn-sm" style="background:rgba(13,110,253,0.15);color:#0d6efd;">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="/PROYECTO-tecnosoluciones-sgp/public/tickets/edit/<?= $t['id'] ?>"
                                       class="btn btn-sm" style="background:rgba(255,193,7,0.15);color:#ffc107;">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <button onclick="deleteTicket(<?= $t['id'] ?>, '<?= htmlspecialchars($t['titulo']) ?>')"
...
function deleteTicket(id, titulo) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `/PROYECTO-tecnosoluciones-sgp/public/tickets/delete/${id}`;
        }
    });
}
</script>
